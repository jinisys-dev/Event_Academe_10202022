...
{
	public class NonGuestTransactionDAO
	{

		private MySqlConnection localConnetion;
		private int hotelId;

		public NonGuestTransactionDAO()
		{
			localConnetion = GlobalVariables.gPersistentConnection;
			hotelId = GlobalVariables.gHotelId;
		}

		public DataTable getNonGuestTransactions(DateTime pTransactionDate)
		{
			DataTable dtTransactions = new DataTable();
			try 
			{
				string strQuery = "call spSelectNonGuestTransactions('" + string.Format("{0:yyyy-MM-dd}",pTransactionDate) + "')";

				MySqlDataAdapter dtaSelect = new MySqlDataAdapter(strQuery, localConnetion);
		...
		public string appendNonGuestTransaction(object newTransaction, ref MySqlTransaction trans)
		{
		...
		public void voidNonGuestTransaction(object newTransaction, ref MySqlTransaction trans)
		{
			...
			string strQuery = "call spVoidNonGuestTransaction('" + transactionId
															+ "','" + updatedBy 
															+ "','" + hotelId 
														    + "'," + baseAmount 
															+ ",'" + referenceDriverId + "')";
